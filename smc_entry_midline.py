"""
Smart Money Concepts - Entrada na Linha do Meio do Order Block
==============================================================
Entrada EXATA no meio do OB (linha azul) quando o candle TOCA esse preço
Stop Loss e Take Profit projetados a partir do preço de entrada
"""
...
class TradeSignal:
    ...
    entry_price: float            # Preço de entrada (MEIO do OB)
...
class SMCMidline:
    """
    Smart Money Concepts com entrada na linha do meio do OB
    """
...
class OrderBlockStrategyMidline:
    """
    Estratégia de Order Block com entrada na LINHA DO MEIO do OB.
    O candle deve TOCAR o preço do meio para ativar a entrada.
    """
    ...
    def __init__(
        ...
        self.swings = SMCMidline.swing_highs_lows(self.ohlc, swing_length)
        self.order_blocks = SMCMidline.order_blocks(self.ohlc, swing_length)
    ...
    def generate_signals(self) -> List[TradeSignal]:
        """
        Gera sinais de trade com entrada na LINHA DO MEIO do OB.
        O candle deve TOCAR o preço do meio para ativar a entrada.
        """
        signals = []
        n = len(self.ohlc)
        ...
        for idx in ob_indices:
            ...
            # Calcular linha do meio (LINHA AZUL)
            ...
            for j in range(entry_start, min(n, i + 100)):
                ...
                # Bullish OB - preço deve TOCAR a linha do meio
                if ob_direction == 1:
                    # Verificar se o candle TOCOU a linha do meio
                    if current_low <= midline <= current_high:
                        # ENTRADA EXATA NA LINHA DO MEIO
                        entry_price = midline
                        
                        # Stop Loss abaixo do fundo do OB
                        stop_loss = ob_bottom - (ob_top - ob_bottom) * 0.1
                        
                        # Calcular risco a partir do preço de entrada
                ...
                # Bearish OB - preço deve TOCAR a linha do meio
                elif ob_direction == -1:
                    # Verificar se o candle TOCOU a linha do meio
                    if current_low <= midline <= current_high:
                        # ENTRADA EXATA NA LINHA DO MEIO
    ...
    def backtest(self, signals: Optional[List[TradeSignal]] = None) -> Tuple[List[BacktestResult], Dict]:
        """
        Executa backtest.
        Verifica TP/SL APENAS a partir do PRÓXIMO candle após a entrada.
        Isso é mais realista pois a ordem limit é preenchida no candle de entrada.
        """
        if signals is None:
        ...
        for signal in signals:
            ...
            # Verificar TP/SL APENAS a partir do PRÓXIMO candle
            # A entrada ocorre no candle atual, então só podemos sair no próximo
...
def test_strategy():
    ...
    print("=" * 70)
    print("TESTE - ENTRADA NA LINHA DO MEIO DO OB (LINHA AZUL)")
    print("=" * 70)
    ...
    print("\nLógica: Entrada quando o candle TOCA a linha do meio do OB")
    print("        Preço de entrada = (ob_top + ob_bottom) / 2")
